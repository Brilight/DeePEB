...
class DeePEB(nn.Module):
    def __init__(self, devices, modes1, modes2, modes3, channels, layers, hf_channels):
        ...
        self.layers = layers
        self.device0, self.device1 = devices[0], devices[1]
        
        self.dsp = nn.Sequential(
            nn.Conv3d(1, 1, kernel_size=3, stride=(1,2,2), padding=1, padding_mode='replicate')).to(self.device0)

        self.fn0 = nn.Sequential(nn.Linear(1, self.channels)).to(self.device0)
        ...
        self.fn1 = nn.Sequential(nn.Linear(self.channels, 64)).to(self.device0)
        self.fn2 = nn.Sequential(nn.Linear(64, 1)).to(self.device1)
        ...
        self.hf0 = nn.Sequential(
            ...
            nn.Conv3d(hf_channels, 1, kernel_size=1, stride=1)).to(self.device1)
        ...
        self.hf1 = nn.Sequential(
            ...
            nn.LeakyReLU(inplace=True),
            nn.Conv3d(1, 1, kernel_size=3, stride=1,
                      dilation=1, padding=1, padding_mode='replicate')).to(self.device1)
        ...
        self.hf2 = nn.Sequential(
            ...
            nn.LeakyReLU(inplace=True),
            nn.Conv3d(1, 1, kernel_size=3, stride=1,
                      dilation=2, padding=2, padding_mode='replicate')).to(self.device1)
        ...
        self.hf3 = nn.Sequential(
            ...
            nn.LeakyReLU(inplace=True),
            nn.Conv3d(1, 1, kernel_size=(1,5,5), stride=1,
                      dilation=1, padding=(0,2,2), padding_mode='replicate')).to(self.device1)
    ...
    def forward(self, acd):
        ...
        x = self.activation(x).to(self.device1)
        x = self.fn2(x).permute(0, 4, 1, 2, 3)
        x = self.usp(x)
        acd = acd.to(self.device1)
