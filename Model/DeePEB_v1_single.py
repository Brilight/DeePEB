...
class DeePEB(nn.Module):
    def __init__(self, modes1, modes2, modes3, channels, layers, hf_channels):
        ...
        self.hf2 = nn.Sequential(
            nn.Conv3d(1, 2, kernel_size=3, stride=1, 
                      dilation=2, padding=2, padding_mode='replicate'),
            nn.LeakyReLU(inplace=True),
            nn.Conv3d(2, 1, kernel_size=3, stride=1,
                      dilation=2, padding=2, padding_mode='replicate'))
        
        self.hf3 = nn.Sequential(
            nn.Conv3d(1, 1, kernel_size=(1,5,5), stride=1, 
                      dilation=1, padding=(0,2,2), padding_mode='zeros'),
            nn.LeakyReLU(inplace=True),
            nn.Conv3d(1, 1, kernel_size=(1,5,5), stride=1,
                      dilation=1, padding=(0,2,2), padding_mode='zeros'))
    ...
    def forward(self, acd):
        ...
        x = ckpt(self.fn1, x.permute(0, 2, 3, 4, 1))
        x = self.activation(x)
        x = self.fn2(x).permute(0, 4, 1, 2, 3)
        x = self.usp(x)
        acd.requires_grad_(True)
        x += self.hf0(acd) + ckpt(self.hf1, acd) + ckpt(self.hf2, acd) + ckpt(self.hf3, acd)
        return x